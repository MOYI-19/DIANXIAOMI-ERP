...
class App(ctk.CTk):
    ...
    def _build_log_area(self, parent):
        ...
        self.debug_btn.grid(row=0, column=1, padx=(0, 4), sticky="e")
    ...
    def _on_launch_result(self, result, exc):
        ...
        self.debug_btn.configure(state="normal")  # 启用调试按钮
    ...
    def _on_ready(self):
        ...
        self._async_call(
            self._run_full_automation(self._current_store, self._current_start),
    ...
    def _on_success(self, result: dict):
        ...
        if remaining:
            ...
            self.log("")
            self.log("=" * 55)
            self.log(f"👉 请处理下一个店铺: {STORES[next_store]['name']}")
            self.log(f"   下一个起始单号应为: {next_start}")
            self.log(f"   在应用中选择「{next_store}」并填入新起始号，然后重新启动浏览器")
    ...
    def _prompt_next_store(self, next_store: str, next_start: int):
        ...
        if result:
            ...
            self.set_status(f"📋 已填入「{STORES[next_store]['name']}」信息，请点击启动浏览器")
        else:
            self._reset_ui()
